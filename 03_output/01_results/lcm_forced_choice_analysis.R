# resources
#http://www.apollochoicemodelling.com/files/Apollo_example_18.r 
###A model with two classes, where all four marginal utility paramters vary across classes, but ASCs are kept fixed across classes
#http://www.apollochoicemodelling.com/files/Apollo_example_20.r
###A class allocation model, where socio-demographic characterisitics
# ####################################################### #
...
apollo_control<-list(
  ...
  modelDescr="LCM model on SP data for PA Program data",
  indivID= "ID",
  nCores=2,
  noDiagnostics=TRUE
)
...
apollo_lcPars=function(apollo_beta, apollo_inputs){
  lcpars = list()
#produce one element in the list for each of the four marginal utility coefficients
  #Each of the elemnts is a list in itself, and contains the values for the coefficients across classes
  #more classes used, more entries added, where possibility exists of keeping alues constant for some params across
  #some or all of the classes (in which case the # of calues still needs to be the same as the number of classes, but some are repeated)
  lcpars[["b_goal"]] = list(b_goal_30, b_goal_60, b_goal_90)
  lcpars[["b_form"]] = list(b_form_cash, b_form_donate,b_form_voucher)
  lcpars[["b_mag"]] = list(b_mag_160, b_mag_300,b_mag_500)
  lcpars[["b_dir"]] = list(b_dir_pos, b_dir_neg)
  
  V=list()
  V[["class_a"]] = delta_a
  V[["class_b"]] = delta_b

#We next calculate class allocation probabilities. 
#We use a MNL model for class allocation probabilities, and thus produce utility duncion
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  while(s<=2){
    ...
    V=list()
    V[['alt1']]  = asc_1 + b_goal[[s]]*goal1 + 
      #b_form[[s]]*form1 + #
      b_mag[[s]]*mag1 
    #+ b_dir[[s]]*dir1
    V[['alt2']]  = asc_2 + b_goal[[s]]*goal2 + 
      #b_form[[s]]*goal2 +# 
      b_mag[[s]]*mag2 
    #+ b_dir[[s]]*dir2
